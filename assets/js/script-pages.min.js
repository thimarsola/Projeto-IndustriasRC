...
item.preloaded=!0}}});var RETINA_NS='retina';$.magnificPopup.registerModule(RETINA_NS,{options:{replaceSrc:function(item){return item.src.replace(/\.\w+$/,function(m){return'@2x'+m})},ratio:1},proto:{initRetina:function(){if(window.devicePixelRatio>1){var st=mfp.st.retina,ratio=st.ratio;ratio=!isNaN(ratio)?ratio:ratio();if(ratio>1){_mfpOn('ImageHasSize'+'.'+RETINA_NS,function(e,item){item.img.css({'max-width':item.img[0].naturalWidth/ratio,'width':'100%'})});_mfpOn('ElementParse'+'.'+RETINA_NS,function(e,item){item.src=st.replaceSrc(item,ratio)})}}}}});_checkInstance()}));$(document).ready(function(){const path=$(location).attr("href").split("contato/",1).reverse().join('');const file='wp-content/themes/industriasrc/source/Support/Sender.php';$('#form').submit(function(){$(".contact__container__row__form__content__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__container__row__form__content__status').append(data);setTimeout(function(){$(".contact__container__row__form__content__status").addClass("d-none")},3000)},error:function(){$('.contact__container__row__form__content__status').append('Erro')}});return!1})});/*!
 * sweetalert2 v7.0.8
